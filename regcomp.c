...
e3(Parselex *plex)
{
	...
	default:
		if(plex->rune)
			snprintf(error, sizeof(error), "regexp %s: syntax error: %lc", plex->orig, plex->rune);
		else
...
static int
utflen(char *s) {
	int numbytes, numrunes=0;
	for ( ; *s ; s+=numbytes ) {

		numbytes = mblen(s, MB_CUR_MAX);
...
getnextr(Parselex *l)
{
	...
	// TODO: THIS IS BAD
	l->rawexp += mbtowc(&l->rune, l->rawexp, MB_CUR_MAX);
	if(*l->rawexp == 0)
		l->done = 1;
	if(l->rune == L'\\')
		getnextrlit(l);
...
buildclass(Parselex *l)
{
	...
	i = 0;
	n = node(l, TCLASS, NULL, NULL);
	// FIXME: we're assuming the encoding is Unicode...
	n->r = 0x10FFFF + 1;
	n->nclass = i++;
